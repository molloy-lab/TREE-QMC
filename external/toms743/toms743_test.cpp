...
//
{
  ...
  toms743::timestamp ( );
  std::cout << "\n";
  std::cout << "TOMS743_TEST\n";
  std::cout << "  C++ version\n";
  std::cout << "  Test the TOMS743 library.\n";

  nbits =  toms743::nbits_compute ( );
  std::cout << "\n";
...
//
  ...
  for ( i = 0; i < 68; i++ )
  {
    ...
    {
      em = - exp ( -1.0 );
      w = toms743::wapr ( em, 0, nerror, 0 );
    }
    else
    {
      w = toms743::wapr ( dx1[i], 0, nerror, 1 );
    }
...
//
  ...
  for ( i = 0; i < 20; i++ )
  {
    w = toms743::wapr ( x2[i], 0, nerror, 0 );
    if ( w == wp2[i] )
    {
      nd = ( int ) ( log10 ( pow ( 2.0, nbits ) ) + 0.5 );
    }
...
//
  ...
  for ( i = 0; i < 10; i++ )
  {
    w = toms743::wapr ( x3[i], 0, nerror, 0 );
    if ( w == wp3[i] )
    {
      nd = ( int ) ( log10 ( pow ( 2.0, nbits ) ) + 0.5 );
    }
...
//
  ...
  for ( i = 0; i < 68; i++ )
  {
    w = toms743::wapr ( dx1[i], 1, nerror, 1 );
    if ( w == wm1[i] )
    {
      nd = ( int ) ( log10 ( pow ( 2.0, nbits ) ) + 0.5 );
    }
...
//
  ...
  for ( i = 0; i < 68; i++ )
  {
    if ( 0.0 < dx1[i] )
    {
      w = toms743::wapr ( -dx1[i], 1, nerror, 0 );
      if ( w == wm2[i] )
      {
        nd = ( int ) ( log10 ( pow ( 2.0, nbits ) ) + 0.5 );
      }
